...
interface Order {
  ...
  subtotal: number;
  vat_amount: number;
  items: OrderItem[]; // Fixed: This needs to be an array type to allow .map() and .length
  created_at: string;
...
function OrderDetail() {
  ...
  const [history, setHistory] = useState<HistoryEntry[]>([]); // Fixed: Initialized as an empty array
  ...
  const load = async () => {
    const { data: o } = await supabase
      ...
      .eq("id", orderId)
      .maybeSingle(); // Fixed: Added execution ()
    ...
    setHistory((h ?? []) as HistoryEntry[]); // Fixed: Handled empty array fallback
    setLoading(false);
  };

  useEffect(() => {
    load(); // Fixed: Executed function
  }, [orderId]);

  const updateStatus = async (status: string) => {
    if (!order) return;
    setUpdating(true);
    const { data: u } = await supabase.auth.getUser(); // Fixed: Added execution ()
  ...
  const reorder = async () => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        total: order.total,
        items: [], // Fixed: Empty array syntax
        status: "pending",
        payment_status: "unpaid",
      })
      .select() // Fixed: Added execution ()
      .single();
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2 border-t border-border pt-3 text-right">
            <div className="text-xs text-muted-foreground">
              Subtotal ${Number(order.subtotal).toFixed(2)} · VAT ${Number(order.vat_amount ?? 0).toFixed(2)}
            </div>
