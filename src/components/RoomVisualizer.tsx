 ...
 *
 * One color for the whole room: tapping an unpainted spot adds that
 * connected surface to the set of "wall" patches; tapping an already-
 * painted patch removes just that patch. Every patch always shares the
 * same currently-selected color, tolerance, and strength — there is no
 * per-wall color anymore, since the goal is a single repaint of the room,
 * not comparing colors wall-to-wall.
 */
export function RoomVisualizer() {
  ...
  const buildMaskFor = useCallback((seed: { x: number; y: number }, toleranceValue: number) => {
    ...
    // A wall in a room photo is never the entire photo, so growth is capped
    // both by distance from the tapped point and by total area, independent
    // of how the edge check behaves.
    const seedX = seed.x;
    const seedY = seed.y;
  ...
  const tapCanvas = (event: MouseEvent<HTMLCanvasElement>) => {
    ...
    // Tapping inside an already-painted patch removes just that patch.
  ...
  const updateTolerance = (value: number) => {
    setTolerance(value);
    // Tolerance is shared by every patch, so a change re-runs the fill from
    // each patch's original tap point rather than only affecting new taps.
