...
import { Download, Eye, EyeOff, ImagePlus, Loader2 } from "lucide-react";
 ...
 * Room Visualizer
 *
 * Tints the whole uploaded photo in the selected Sunburst color at once —
 * no wall detection, no tap-to-select. Each pixel's own luminance is
 * preserved so shadows, highlights, and texture still read through the new
 * color rather than flattening the photo into a solid block. Everything
 * runs on-device (the photo is decoded straight into a <canvas> and never
 * uploaded anywhere).
 */
export function RoomVisualizer() {
  ...
  }, [showPaint, selectedColor, strength]);
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_300px]">
      <section className="space-y-3">
        <div className="relative flex min-h-80 items-center justify-center overflow-hidden rounded-lg border border-border bg-muted/40">
          ...
          <canvas ref={previewCanvasRef} className={`max-h-[620px] w-full object-contain ${photoLoaded ? "" : "hidden"}`} aria-label="Room color preview" />
          {!photoLoaded && (
            <label className="flex min-h-80 w-full cursor-pointer flex-col items-center justify-center gap-3 text-center">
              ...
              <span className="max-w-xs text-sm text-muted-foreground">
                The whole photo is repainted in the selected color. Your photo stays on this device.
              </span>
        ...
        {photoLoaded && (
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => setShowPaint((visible) => !visible)}>
              {showPaint ? <EyeOff className="mr-1.5 h-4 w-4" /> : <Eye className="mr-1.5 h-4 w-4" />}
              {showPaint ? "Before" : "After"}
            </Button>
            <Button size="sm" onClick={download} className="ml-auto">
        ...
        <p className="text-sm text-muted-foreground">
          {photoLoaded
            ? "The whole photo updates to whichever color you pick — pick one to preview it."
      ...
      <aside className="space-y-5">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Applies to the whole photo.</p>
        </div>
