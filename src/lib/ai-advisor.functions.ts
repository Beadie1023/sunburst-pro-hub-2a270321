...
export const recommendColors = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Use environment variable for Render backend with fallback to production endpoint
    const renderApiUrl = process.env.VITE_RENDER_API_URL || "https://sunburst-b88c.onrender.com";
